...
const GenericPrint: React.FC<GenericPrintProps> = ({
  ...
  return (
    <div className="bg-white p-2 mb-2 rounded-lg max-w-4xl mx-auto border border-gray-200 print:border-0  print:pr-1 print:pl-14 print:max-w-full font-abyssinica">
      ...
      {(documentType === "dividend" || documentType === "payment") && (
        <div className="flex items-center gap-4 tracking-wide flex-wrap  w-fit  pl-12">
      ...
      {(documentType === "dividend" || documentType === "payment") && (
        <div className="max-w-2xl mx-auto mb-2 pl-5 pr-52 pt-6 pb-8">
          <div className="grid grid-cols-2 gap-y-2 gap-x-4 text-base text-center">
            ...
            {/* Dividend */}
            <span className="font-semibold text-left text-nowrap ">
              መ. የትርፍ ድርሻ (ከታክስ በፊት):
            </span>
            <span className="border-b-2 border-black text-center pl-2">
              {Intl.NumberFormat("en-US", {
      ...
      {/* Notices (Unchanged) */}
      {(documentType === "dividend" || documentType === "payment") && (
        <div className="mb-4 p-2 rounded mt-1 print:mt-2 print:text-xs">
